...
class pixel_simularity:
    ...
    def cal_sim(self,threshold):
        res = []
        a = self.before_changed
        b = self.after_changed
        for i in zip(a,b):
            a = next(iter(i))
            b = next(iter(i))
            # print(self.before_img.shape)
            # print(self.after_img.shape)
            #x,y,w,h
            ...
            # plt.show()
            rgb_hist.append(temp)
            print(self.dot_product(rgb_hist[0],rgb_hist[1]))
            if self.dot_product(rgb_hist[0],rgb_hist[1]) >= threshold:
                res.append(True)
            else:
                res.append(False)
        return res